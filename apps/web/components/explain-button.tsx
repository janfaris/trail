...
function Callout({ children }: { children: React.ReactNode }) {
  return (
    <div className="border border-white/10 bg-zinc-900/40 rounded-md p-5 my-8 animate-in fade-in duration-300">
      <div className="text-[11px] font-mono uppercase tracking-wider text-[var(--accent-text)] mb-2">
        ★ AI summary
      </div>
  ...
  return (
    <div className="my-8">
      <button
        ...
        onClick={onClick}
        disabled={pending}
        className="inline-flex items-center gap-2 h-9 px-4 rounded-md border border-white/10 bg-zinc-900/50 text-sm font-mono text-zinc-200 hover:text-[var(--accent-text)] hover:border-white/20 transition-[color,background-color,border-color,transform] active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {pending ? (
          <>
            <span className="inline-block h-3 w-3 rounded-full border-2 border-white/15 border-t-[var(--accent-border)] animate-spin" />
            Generating…
          </>
        ) : (
